...
int main ( int argc, char **argv )
{
	...
	IplImage* img = cvLoadImage(argv[1]);
		if ( !img )           		// Check for Image Input capture
		return -1;

	cvNamedWindow("Image Input",CV_WINDOW_AUTOSIZE);
	...
	while(key != 'q')		// While loop to query for Image Input frame
	{
		...
		//Step	: Capture Image from Image Input
		//Info	: Inbuit function from OpenCV
		//Note	: 

		//Step	: Convert Image captured from Image Input to GrayScale
		//Info	: Inbuit function from OpenCV
		//Note	: Image from Image Input and Grayscale are held using seperate "IplImage" objects
        ...
        for( int y = 0; y < thres->height; ++y)	//Start full scan of the image by incrementing y
        {
			...
			// Tweaking blob detection
                        if(n > 10)
                        {
                            if(aspect_ratio > 0)	
                            {

                                	// Its a considerable Blob, Mark it
									cvRectangle(img,cornerA,cornerB,CV_RGB(255,0,0), 2);
                            }	
		...
		cvShowImage("Image Input",img);
		//cvShowImage("Threshold",thres);
		key = cvWaitKey(0);	// OPENCV: wait for 1ms before accessing next frame

	}	// End of 'while' loop

	cvDestroyWindow( "Image Input" );	// Release various parameters
	cvReleaseImage(&img);
	cvReleaseImage(&gray);
